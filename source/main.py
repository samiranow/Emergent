...
API_ENDPOINT = "https://your-api.example.com/ping"  # ← جایگزین با API خودت
...
def extract_host(line: str, proto: str):
    try:
        if proto == "vmess":
            b64_part = line[8:]  # ← درست
...
async def test_speed(host: str, timeout=5):
    ...
    # ارسال فقط هاست به API
    try:
        requests.post(API_ENDPOINT, json={"host": host, "latency_ms": latency_ms}, timeout=5)
    except Exception as e:
        print(f"⚠️ Error sending to API for {host}: {e}")
